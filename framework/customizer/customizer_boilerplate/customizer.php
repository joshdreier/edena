...
function thsp_cbp_customize_register( $wp_customize ) {
	...
	foreach( $thsp_sections as $thsp_section_key => $thsp_section_value ) {
		...
		/* build typo */
		$arrayFontWeightTotal = array();

		//delete_transient( 'neko_array_font_weight_total' );
		$cached_font_weight_total = get_transient( 'neko_array_font_weight_total' );
		if( empty( $cached_font_weight_total ) ){
			...
			foreach ($regfonts_array['items'] as $key => $value) {
				foreach ($value['variants'] as $keyV => $valueV) {
					$arrayFontWeightTotal[$value['family']][] = $valueV;
				}
			}	
			/* / REGULAR FONTS */
			...
			$arrayFontWeightTotal = get_transient( 'neko_array_font_weight_total' );
		}
		...
		if( empty($cached_font_weight_total) ){
			set_transient( 'neko_array_font_weight_total', $arrayFontWeightTotal, 0 );
		}


		//echo '<pre>'; print_r($arrayFontWeightTotal); echo '</pre>';



		$wp_customize->add_setting( 'neko_fontWeights', array(
			'default' => $arrayFontWeightTotal,
			'type' => 'option',
		...
		foreach( $thsp_section_fields as $thsp_field_key => $thsp_field_value ) {
			 ...
			 */
			if ( ! isset( $thsp_field_value['setting_args']['sanitize_callback'] ) ) {
				$thsp_field_value['setting_args']['sanitize_callback'] = 'thsp_sanitize_cb';
			}
			...
			$wp_customize->add_setting(
				$setting_control_id,
				array(
					'default' => $thsp_field_value['setting_args']['default'],
					'type' => $thsp_field_value['setting_args']['type'],
					'capability' => $thsp_field_value['setting_args']['capability'],
					'transport' => $thsp_field_value['setting_args']['transport'],
					'sanitize_callback' => $thsp_field_value['setting_args']['sanitize_callback']
					)
			...
			}elseif ( 'sub-title' == $thsp_field_value['control_args']['type'] ) {
				
				$wp_customize->add_control(
					new CBP_Customizer_Sub_Title(
						$wp_customize,
						$setting_control_id,
						$thsp_field_value['control_args']
						)
			...
			}elseif ( 'description' == $thsp_field_value['control_args']['type'] ) {

				$wp_customize->add_control(
					new CBP_Customizer_Description(
						$wp_customize,
						$setting_control_id,
						$thsp_field_value['control_args']
						)
