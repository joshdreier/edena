...
if ( ! class_exists( 'Neko_at_Meta_Box') ) :
...
class Neko_at_Meta_Box {
  ...
  public function check_field_select() {
    ...
    if ( ! $this->has_field( 'select' )) return;
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = get_post_meta($post->ID,$field['id'],true);
      ...
      if (count($meta) > 0 && is_array($meta) ){
         foreach ($meta as $me){
           ...
           $customTitle = get_the_title( $mmm );
           $title = (!empty($customTitle))? $customTitle: $mmm ;
           if ( in_array( $field['fields'][0]['type'], array('image','file') ) )
            $mmm = $c +1 ;
           echo '<div class="at-repater-block"><span class="title">'.$title.'</span><br/><table class="repeater-table" style="display: none;">';
           if ($field['inline']){
        ...
        <span class="re-control"><img src="'.$plugin_path.'/images/remove.png" alt="'.esc_html__('Remove','neko').'" title="'.esc_html__('Remove','neko').'" id="remove-'.$field['id'].'"></span>
    ...
    echo '</table><img src="'.$plugin_path.'/images/remove.png" alt="'.esc_html__('Remove','neko').'" title="'.esc_html__('Remove','neko').'" id="remove-'.$field['id'].'"></div>';
  ...
  public function show_field_posts($field, $meta) {
    ...
    // select
      ...
      if (isset($field['emptylabel']))
        echo '<option value="-1">'.(isset($field['emptylabel'])? $field['emptylabel']: esc_html__('Select ...','neko')).'</option>';
      foreach ($posts as $p) {
  ...
  function Validate_upload_file_type($file) {
    if (isset($_POST['uploadeType']) && !empty($_POST['uploadeType']) && isset($_POST['uploadeType']) && $_POST['uploadeType'] == 'my_meta_box'){
      ...
      if (!in_array($ext, (array)$allowed)){
        $file['error'] = esc_html__("Sorry, you cannot upload this file type for this field.", 'neko');
        return $file;
      }
