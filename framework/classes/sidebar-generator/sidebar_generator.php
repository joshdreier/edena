...
class sidebar_generator {
	...
	function sidebar_generator(){
		...
		if ( is_admin() ){
			...
			$post_type = '';
			/* if post id check the real post type */
			if( !empty($post_id) ){
				$post_type = get_post_type($post_id) ;

			/* get post type by get param */	
			}elseif( !empty($_GET['post_type']) ){
				$post_type = $_GET['post_type'] ;

			/* get post type if adding a new post (no get params and no post id) */		
			}elseif( preg_match('/post-new/', $_SERVER['REQUEST_URI']) && empty($_GET['post_type']) ){
				$post_type = 'post' ;
			}

			/* Check for visual composer by post id or if vc is enabled */
			$vc_enabled = neko_check_vc_status( $post_id );

			if( empty($vc_enabled) && ( $post_type =='page' || $post_type == 'post' ) ) {
				//edit posts/pages
					...
					if(is_array($sidebars) && !empty($sidebars)){
						...
						foreach($sidebars as $sidebar){
							...
							<tr class="<?php echo esc_attr( $alt );?>">
								...
								<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo esc_js( $sidebar ); ?>',<?php echo intval( $cnt+1 ); ?>);" title="Remove this sidebar">remove</a></td>
							</tr>
							<?php
							$cnt++;
						}
	...
	*/
	public static function save_form($post_id){
		...
		if(!empty($is_saving)){
			...
			delete_post_meta($post_id, 'neko_sidebar_position');
			...
			add_post_meta($post_id, 'neko_sidebar_position', $_POST['neko_sidebar_position']);
		}		
	}
	...
	public static function edit_form(){
		...
		$selected_sidebar_pos = get_post_meta($post_id, 'neko_sidebar_position', true);
		if(!is_array($selected_sidebar_pos)){
			$tmp = $selected_sidebar_pos; 
			$selected_sidebar_pos = array(); 
			$selected_sidebar_pos[0] = $tmp;
		}
		?>

		<div id='sbg-sortables' class='meta-box-sortables'>
			<div id="sbg_box" class="postbox " >
				<div class="handlediv" title="Click to toggle"><br /></div><h3 class='hndle'><span><?php esc_html_e('Sidebar management', 'neko'); ?></span></h3>
				<div class="inside">
					<div class="sbg_container">
						<input name="sbg_edit" type="hidden" value="sbg_edit" />

						<p>
						<?php esc_html_e('Please select the sidebar and its position you would like to display on this page / post. Note: You must first create the sidebar under Appearance > Sidebars.', 'neko'); 
						?>
						</p>
						...
						<ul>
							...
							for($i=0;$i<1;$i++){ ?>
							<li>
								<select name="sidebar_generator[<?php echo intval( $i )?>]" style="display: none;">
						...
						<?php if( function_exists('is_woocommerce') && get_option('woocommerce_shop_page_id') != $post_id || !function_exists('is_woocommerce') ) { ?>
						<label class="neko-metabox-label"><?php esc_html_e('Sidebar Name', 'neko');  ?></label>
						<select name="sidebar_generator_replacement[<?php echo intval( $i )?>]" class="neko-metabox-select">

							<?php
								if('post' === $post_type){
									$default_selected = ( !empty($selected_sidebar_replacement[$i]) && 'blog-default' == $selected_sidebar_replacement[0] ) ? 'selected=selected' : '' ;
									echo "<option value='blog-default' ".esc_attr($default_selected).">". esc_html__('Blog default','neko')."</option>\n";
								}
							?>

							<?php
								$nosidebar_selected = ( !empty($selected_sidebar_replacement[$i]) && 'no-sidebar' == $selected_sidebar_replacement[0] ) ? 'selected=selected' : '' ;
							?>
							<option value="no-sidebar"  <?php echo esc_attr($nosidebar_selected); ?> ><?php esc_html_e('No sidebar', 'neko'); ?></option>
							<?php
								...
								if(is_array($sidebar_replacements) && !empty($sidebar_replacements)){
									...
									foreach($sidebar_replacements as $sidebar){
										...
										if('post' === $post_type){
											if( 'Blog sidebar' != $sidebar['name'] ){
												if($selected_sidebar_replacement[$i] == $sidebar['name']){
													echo "<option value='{$sidebar['name']}' selected=\"selected\">{$sidebar['name']}</option>\n";
												}else{
													echo "<option value='{$sidebar['name']}'>{$sidebar['name']}</option>\n";
												}
											}
						...
						<?php } ?>
							...
							<?php 
								$sidebarPos = array(
									'no-sidebar' => esc_html__('No position','neko'),
									'left'       => esc_html__('Left','neko'),
									'right'      => esc_html__('Right','neko')
								);

								if('post' === $post_type){
									$array_post_new_value = array('blog-default' => esc_html__('Blog default','neko') );
									$sidebarPos = $array_post_new_value + $sidebarPos;
								}

							?>
							<label class="neko-metabox-label"><?php esc_html_e('Sidebar Position', 'neko');?></label>
							<select name="neko_sidebar_position[<?php echo intval( $i )?>]" class="neko-metabox-select">
								<?php 
								foreach ($sidebarPos as $key => $value) {
									$selectedPos = ( $key ==  $selected_sidebar_pos[$i])?'selected="selected"':'';
									echo '<option value="'.$key.'" '.$selectedPos.'>'.$value.'</option>';
								}
								?>
							</select> 							
			...
			if(is_array($selected_sidebar) && !empty($selected_sidebar)){
				...
				for($i=0;$i<sizeof($selected_sidebar);$i++){					
					...
					if($name == "0" && $selected_sidebar[$i] == "0" &&  $selected_sidebar_replacement[$i] == "0"){
						...
						//echo "\n\n<!-- [called $name selected {$selected_sidebar[$i]} replacement {$selected_sidebar_replacement[$i]}] -->";
						if("blog-default" == $selected_sidebar_replacement[$i]){
							dynamic_sidebar('sidebar-1');//default behavior
							$did_sidebar = true;
							break;
						}else{
